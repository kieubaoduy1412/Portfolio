...
function loginToBlog(){
	$.ajax({
		...
		if(data.IS_ALLOWED){
			...
			$(".alert-message").html("");
			$(".alert-message").append("Wrong password")
		}
    });
}
...
setInterval(function() {
	//fade In - fade Out
	if(imageChanged == 0){
		$('header').css("background-image", "url(webapp/img/background2.jpg)"); 
		imageChanged = 1;
	} else {
		$('header').css("background-image", "url(webapp/img/background.jpg)"); 
		imageChanged = 0;
	}
}, 5000);